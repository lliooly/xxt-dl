...
XXT DL 未来会从单体下载工具演进为多入口题库系统。本地 Web 负责用户主流程，云端负责移动训练入口，本地和云端通过清晰的数据模型连接。
...
| `web` | 本地任务服务、Route Handlers、GUI 状态 | 具体题目解析规则 |
...
云端阶段不直接托管学习通登录态。本地服务负责采集和本地入库，用户明确开启云同步后，再把题库、练习记录或必要索引同步到云端。
...
1. 保持现有 Web、CLI、Playwright 和导出能力可运行。
2. 先新增模块目录和文档，再迁移实现。
