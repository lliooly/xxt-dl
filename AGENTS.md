...
当前仍有早期文件留在 `src/browser.ts`、`src/clean.ts`、`src/core.ts` 和 `src/cli.ts`。迁移时应保持兼容出口，避免一次性重排导致 CLI、Web 或测试失效。

## 工作规则

- GUI 是普通用户主入口；CLI / TUI 只作为开发、调试和自动化入口。
- 抓取结果未来应进入结构化题库；Markdown、HTML、JSON 是导出物，不应长期作为主状态。
- 云端和 Bot 代码不能直接依赖 Playwright 或本地浏览器 profile。
...
- Web 层应调用模块 API，不要把解析、题库或练习逻辑写死在 React 页面里。
...
- Next.js 相关改动：运行 `npm run build`，并尽量手动检查本地 Web 界面。
- CI 流程改动：检查 `.github/workflows/ci.yml`。
